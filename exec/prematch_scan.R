...
df_prematch <- df_prematch %>%
  ...
  dplyr::rowwise() %>%
  dplyr::mutate(competitorFavori   = substr(names(.)[c(8,11)][which.min(c_across(c(competitor1OddPreMatch,competitor2OddPreMatch)))],11,11),
                competitorOutsider = substr(names(.)[c(8,11)][which.max(c_across(c(competitor1OddPreMatch,competitor2OddPreMatch)))],11,11))
